...
int main() {
    ...
    printf("Process\t  Start Time\tCompletion Time\t Turn Around Time  Waiting Time\t Response Time\t\n");
    
    for(int i=0;i<n;i++){
        printf("%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",p[i].pid,p[i].start_time,p[i].completion_time,p[i].turnaround_time,p[i].waiting_time,p[i].response_time);
    }
    printf("Average Turnaround Time = %0.2f \n",avg_turnaround_time);
    printf("Average Waiting Time = %0.2f \n",avg_waiting_time);
    printf("Average Response Time = %0.2f \n",avg_response_time);
    printf("CPU Utilization = %0.2f%% \n",cpu_utilisation);
    printf("Throughput = %0.2f process/unit time \n",throughput);
